...
def lbs(
    ...
    # T posed
    Vs = blend_shapes(betas, shapedirs)
    Vp = pose_offsets
    Vps = Vs + Vp
    Vps = torch.concat([Vps, torch.zeros_like(Vps[..., -1:])], dim=-1)
    Tvps = torch.matmul(T, torch.unsqueeze(Vps, dim=-1))[:, :, :3, 0]
    T_posed = T.clone()
    T_posed[..., 0:3, 3] += Tvps
    # v = v_template
    # v = torch.concat([v, torch.ones_like(v[..., -1:])], dim=-1)
    # Tv = torch.matmul(T_posed, torch.unsqueeze(v, dim=-1))[:, :, :3, 0]

    # add T, W as needed by deformer
    # based on vid2avatar lib/smpl/lbs.py
    return {
        'vertices': verts, 
        'joints': J_transformed,
        'T_weighted': T, 
        'W': W, 
        'T': A.view(batch_size, num_joints, 4, 4),
        'v_shaped': v_shaped,
        'v_posed': v_posed,
        'T_posed': T_posed,
