...
def tbn(triangles):
    ...
    n = F.normalize(torch.cross(b - a, c - a), dim=-1) #计算三角形法线
    d = b - a #计算三角形边

    X = F.normalize(torch.cross(d, n), dim=-1) # 计算切线
    Y = F.normalize(torch.cross(d, X), dim=-1) # 计算副法线
...
def calc_per_face_Rt(cano_triangles, deform_triangles):
    # c2w for triangles
    cano_Rt = triangle2projection(cano_triangles)[0]  # 三角形的[切线，副法线，法线]定义三角形的旋转矩阵
    deform_Rt = triangle2projection(deform_triangles)[0]

    # deform_Rt =  X_d @ cano_Rt
...
class PerVertQuaternion(nn.Module):
    ...
    def calc_per_face_quaternion(self, mesh_verts):
        per_face_Rt = self.calc_per_face_Rt(mesh_verts) # 计算出从cano到deform的旋转矩阵
        per_face_quat = rotation_matrix_to_quaternion(per_face_Rt[:, :3, :3]) # 变成四元组
