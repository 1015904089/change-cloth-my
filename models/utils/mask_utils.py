...
def crop_img(img, h, w):
    ...
    delta_h = int(ori_h - h)
    delta_w = int(ori_w - w)
    return img[:, :, :, int(delta_w / 2):-int(delta_w / 2)]
...
def get_mask_location2(model_type, category, model_parse: Image.Image, keypoint: dict, width=384, height=512):
    im_parse = model_parse.resize((width, height), Image.NEAREST)
    parse_array = np.array(im_parse)
    ...
    if category == 'dresses' or category == 'upper_body':
        ...
        # arm_mask = cv2.dilate(np.logical_or(im_arms_left, im_arms_right).astype('float32'), np.ones((1, 1), np.uint16),
        #                       iterations=1)
        # parse_mask += np.logical_or(parse_mask, arm_mask)
