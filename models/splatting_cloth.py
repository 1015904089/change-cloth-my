...
class SplattingClothModel(GaussianBase):
    ...
    def create_from_mesh(self, mesh, sample_fidxs=None, sample_bary=None):
        ...
        faces = torch.tensor(mesh.faces).long().to(self.device)
        mesh_py3d = py3d_meshes.Meshes(verts.unsqueeze(0), faces.unsqueeze(0))
        norms = torch.tensor(mesh_py3d.verts_normals_packed()).to(self.device)

        self.quat_helper = PerVertQuaternion(verts, faces).to(self.device)
        # phong surface for triangle walk
        self.phongsurf = PhongSurfacePy3d(verts, faces, norms,
                                          outer_loop=2, inner_loop=50, method='uvd').to(self.device)

        self.cano_verts = verts
    ...
    def update_to_posed_mesh(self, mesh=None):
        if mesh is not None:
            ...
            self.tri_quats = self.per_vert_quat[self.cano_faces]  # 面上每个点的旋转四元数

        self._face_scaling = self.quat_helper.calc_face_area_change(self.mesh_verts)  # 得到sclae
