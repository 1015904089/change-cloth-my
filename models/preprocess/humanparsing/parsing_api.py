...
def onnx_inference(session, lip_session, input_dir):
    ...
    with torch.no_grad():
        for _, batch in enumerate(dataloader):
            ...
            parsing_result = np.argmax(logits_result, axis=2)
            parsing_result = np.pad(parsing_result, pad_width=1, mode='constant', constant_values=0)
            parsing_without_hole = parsing_result
            ...
            parsing_without_hole = parsing_without_hole[1:-1, 1:-1]
        ...
        with torch.no_grad():
            for _, batch in enumerate(dataloader_lip):
                image, meta = batch
                c = meta['center'].numpy()[0]
                s = meta['scale'].numpy()[0]
                w = meta['width'].numpy()[0]
    ...
    parsing_without_hole = np.where(neck_mask, 18, parsing_without_hole)
    palette = get_palette(19)
    output_img = Image.fromarray(np.asarray(parsing_result, dtype=np.uint8))
    parsing_without_hole = Image.fromarray(np.asarray(parsing_without_hole, dtype=np.uint8))
    output_img.putpalette(palette)
    parsing_without_hole.putpalette(palette)
    face_mask = torch.from_numpy((parsing_result == 11).astype(np.float32))

    return output_img, parsing_without_hole
